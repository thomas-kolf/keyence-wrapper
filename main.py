...
from failed_process_handler import (
    ...
    copy_failed_input_group,
    verify_failed_input_group_copy,
)
    ...
    for internal_group_id, group_info in groups.items():
        ...
        if raw_problems:
            ...
            report_path = write_failure_report(
                ...
                group_key=group_key,
                verification_type="Raw input completeness failed",
                problems=raw_problems,
            )

            print(f"Failure report written: {report_path.name}")

            failed_input_files = get_available_raw_files_for_failed_group(files)
            ...
            if copy_problems:
                copy_report_path = write_failure_report(
                    failed_process_dir=recipe_failed_dir,
                    group_key=group_key,
                    verification_type="Failed input copy verification failed",
                    problems=copy_problems,
                )

                print(f"Failure copy report written: {copy_report_path.name}")

            else:
                deleted_files = cleanup_processed_group(failed_input_files)

                print(
                    f"Failed group copied to failed_process for {group_key}: "
                    f"{len(copied_files)} files"
                )
                print(
                    f"Deleted processed failed input files for {group_key}: "
                    f"{len(deleted_files)}"
                )
        ...
        if result.valid:
            ...
            if copy_problems:
                ...
                print(
                    f"Deleted processed invalid input files for {group_key}: "
                    f"{len(deleted_files)}"
                )
